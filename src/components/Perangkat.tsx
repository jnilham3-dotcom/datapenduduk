...
import { Perangkat, Penduduk, User } from '../types';
import { api } from '../services/api';
import { motion, AnimatePresence } from 'motion/react';

interface PerangkatProps {
  user: User | null;
}

const PerangkatPage: React.FC<PerangkatProps> = ({ user }) => {
  const [perangkat, setPerangkat] = useState<Perangkat[]>([]);
  const [loading, setLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isDetailOpen, setIsDetailOpen] = useState(false);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<Perangkat | null>(null);
  ...
  const fetchPerangkat = async () => {
    ...
    setError('');
    try {
      const data = await api.getPerangkat();
      setPerangkat(data);
    } catch (err: any) {
  ...
  const handleSearchPenduduk = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setError('Data penduduk tidak ditemukan. Pastikan NIK benar.');
      }
    } catch (err) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!sk || sk.trim().length < 5) {
      setError('Nomor SK wajib diisi (minimal 5 karakter)');
      return;
    }
    if (!tglSk) {
    ...
    try {
      await api.savePerangkat({
        nik: foundPenduduk.nik,
        nama: foundPenduduk.nama_lengkap,
        jabatan,
        sk_pengangkatan: sk,
        tanggal_sk: tglSk
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.deletePerangkat(selectedItem.id);
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Data Perangkat Desa</h1>
          <p className="text-slate-500 mt-1">Kelola struktur organisasi dan perangkat pemerintahan desa.</p>
        </div>
        {hasPermission('action_edit_perangkat') && (
          <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div className="col-span-full bg-white p-12 rounded-3xl shadow-sm border border-slate-100 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-900">Belum ada data perangkat</h3>
            <p className="text-slate-500">Klik tombol "Tambah Perangkat" untuk memulai.</p>
          </div>
        ) : (
            ...
            {perangkat.map((item) => (
              ...
              >
                <div className="absolute top-4 right-4 flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition-all">
                  <button 
                    onClick={() => handleShowDetail(item.nik)}
                    className="p-2 text-slate-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-all"
                    title="Detail Penduduk"
                  >
                    <Eye size={18} />
                  </button>
                  {hasPermission('action_delete_perangkat') && (
                    <button 
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-slate-900 text-lg leading-tight">{item.nama}</h3>
                    <p className="text-emerald-600 font-bold text-sm mt-1">{item.jabatan}</p>
                    <div className="mt-4 space-y-2">
                      ...
                      {item.sk_pengangkatan && (
                        <div className="flex flex-col gap-0.5">
                          <span className="text-[10px] font-bold text-slate-400 uppercase">No. SK</span>
                          <span className="text-xs text-slate-600 font-medium">{item.sk_pengangkatan}</span>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-emerald-500 text-white">
              <h2 className="text-xl font-bold">Tambah Perangkat Desa</h2>
            ...
            <div className="p-8 space-y-6">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1.5">No. SK Pengangkatan</label>
                    <input 
                      type="text" 
                      value={sk}
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1.5">Tanggal SK</label>
                    <input 
                      type="date" 
                      value={tglSk}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-blue-600 text-white">
              <h2 className="text-xl font-bold">Detail Data Perangkat</h2>
      ...
      {isDeleteOpen && selectedItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Hapus Perangkat?</h2>
            <p className="text-slate-500 mb-8">
              Apakah Anda yakin ingin menghapus <strong>{selectedItem.nama}</strong> dari daftar perangkat desa? Peringatan: Tindakan ini tidak dapat dibatalkan.
            </p>
